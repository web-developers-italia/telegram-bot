...
# TTL: elimina i documenti message_reactions ~90gg dopo l'ultimo aggiornamento
# (il campo expiresAt è scritto dal codice come updatedAt + 90gg).
resource "google_firestore_field" "message_reactions_ttl" {
  project    = var.project_id
  database   = google_firestore_database.default.name
  ...
  # Non gestiamo indici su questo campo (non ci interroghiamo sopra;
  # weekly-digest.ts interroga updatedAt, che resta sull'indicizzazione di default).
  index_config {}
}
